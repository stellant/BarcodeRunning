...
namespace Barcode_Keyence
{
    public partial class Form1 : Form
    {
        ...
        private void updateDataValue(byte[] data)
        {
            try
            {
                string dataString = Encoding.GetEncoding("Shift_JIS").GetString(data) + "  " + DateTime.Now.ToString("dd-MMM-yyyy HH:mm:ss:fff");
                WriteData(dataString+"\n");
                textBox_status.AppendText(dataString+"\n");
            }
            catch (Exception ex)
            {
                WriteLog(ex.ToString()+"   at"+DateTime.Now.ToString("dd-MMM-yyyy HH:mm:ss:fff")+"\n");
                textBox_status.AppendText("Exception Occured. Please refer log file."+"\n");
            }
            finally
            {
